...
export default function EsteStoreSidebar() {
  ...
  return (
    <aside
      ...
      className={`shrink-0 sticky top-16 self-start h-[calc(100vh-64px)] bg-[#0F172A]/95 backdrop-blur-md border-r border-slate-800/60 transition-[width] duration-300 ease-out overflow-hidden ${
        open ? 'w-[280px]' : 'w-[72px]'
      }`}
    >
      <div className="h-full overflow-y-auto estestore-sidebar-scroll">
        {/* Sticky header */}
        <div className="sticky top-0 z-10 bg-[#0F172A]/95 backdrop-blur-md border-b border-slate-800/60 h-12 flex items-center px-4">
          {open ? (
