...
export default function HastaSidebar({ userName, pointsBalance, hasClinicAccess, role = 'user' }: Props) {
  ...
  // Mobil drawer açıkken (dokunmatik, hover yok) etiketler görünmeli — yoksa sadece ikon kalır
      ...
      >
        ...
        <div className="border-b border-slate-800 shrink-0">
          <SafeLink href="/panel" className="flex items-center gap-2.5 p-4 group" onClick={() => setMobileOpen(false)}>
            ...
            <div className={`min-w-0 transition-opacity duration-150 ${showLabels ? 'opacity-100' : 'opacity-0 lg:hidden'}`}>
          ...
          <div className="px-4 pb-4">
            <SafeLink
              ...
              className={`flex items-center ${showLabels ? 'justify-between px-3 py-2' : 'justify-center w-9 h-9'} rounded-lg border text-sm transition-all hover:opacity-80 text-amber-400 bg-amber-500/10 border-amber-500/30`}
            ...
            >
              <div className="flex items-center gap-1.5">
                ...
                <span className={`font-bold transition-opacity duration-150 ${showLabels ? 'opacity-100' : 'opacity-0 hidden lg:hidden'}`}>{pointsBalance} Puan</span>
              </div>
            </SafeLink>
          </div>
        </div>
                ...
                {group.items.map(item => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`flex-1 truncate transition-opacity duration-150 ${showLabels ? 'opacity-100' : 'opacity-0 hidden'}`}>{item.label}</span>
                      {showLabels && item.badge === 'soon' && (
        ...
        <div className="p-3 border-t border-slate-800 space-y-1 shrink-0">
          ...
          >
            ...
            <span className={`flex-1 text-left transition-opacity duration-150 ${showLabels ? 'opacity-100' : 'opacity-0 hidden'}`}>
          ...
          {hasClinicAccess && (
            <Link
              href="/klinik/panel"
              className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-sm text-emerald-400 hover:text-emerald-300 hover:bg-emerald-500/10 transition-colors ${showLabels ? '' : 'justify-center'}`}
              title={!expanded ? 'Klinik paneline geç' : undefined}
            >
              <span className="shrink-0">🏥</span>
              <span className={`transition-opacity duration-150 ${showLabels ? 'opacity-100' : 'opacity-0 hidden'}`}>Klinik Paneli</span>
            </Link>
          )}
          <Link
            href="/"
            className={`flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-sm text-slate-500 hover:text-slate-300 hover:bg-slate-800/40 transition-colors ${showLabels ? '' : 'justify-center'}`}
            title={!expanded ? 'Anasayfa' : undefined}
          >
            <span className="shrink-0">⌂</span>
            <span className={`transition-opacity duration-150 ${showLabels ? 'opacity-100' : 'opacity-0 hidden'}`}>Anasayfa</span>
          </Link>
          ...
          <form action={signOutAction}>
            <button
              type="submit"
              className={`w-full flex items-center gap-2 px-2.5 py-1.5 rounded-lg text-sm text-red-400 hover:text-red-300 hover:bg-red-500/10 transition-colors ${showLabels ? '' : 'justify-center'}`}
              title={!expanded ? 'Çıkış Yap' : undefined}
            >
              <svg className="w-3.5 h-3.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
              </svg>
              <span className={`transition-opacity duration-150 ${showLabels ? 'opacity-100' : 'opacity-0 hidden'}`}>Çıkış Yap</span>
            </button>
          </form>
        </div>
      </aside>
