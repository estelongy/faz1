...
export default function CartButton({ className = '' }: { className?: string }) {
  ...
  return (
    <Link href="/sepet"
      ...
      {hydrated && count > 0 && (
        <span className="absolute -top-1 -right-1 bg-violet-600 text-white text-sm font-black rounded-full min-w-[18px] h-[18px] flex items-center justify-center px-1">
          {count}
        </span>
      )}
    </Link>
